...
if (version_compare(phpversion(), '5.3.0', '<')) {
	function eme_phpold() {
		printf('<div class="error"><p>' . __('Your PHP version is too old, please upgrade to a newer version. Your version is %1$s, Events Made Easy requires %2$s', 'eme') . '</p></div>', phpversion(), '5.3.0');
	}
	// Only when we are in the admin, we print a warning
	if (is_admin()) add_action('admin_notices', 'eme_phpold');
	return;
}

// Setting constants
define('EME_DB_VERSION', 84);
...
function eme_insertMyRewriteRules($rules) {
   ...
   $newrules['(.*/)?'.$events_prefix.'town/(.*)'] = 'index.php?page_id='.$events_page_id.'&eme_town=$matches[2]';
...
function eme_explain_dbupdate_done() {
   $advice = sprintf(__("It seems you upgraded Events Made Easy, the events database has been updated accordingly. Click <a href='%s'>here</a> to dismiss this message.",'eme'),add_query_arg(array("eme_disable_update_message"=>"true")));
   ?>
