...
class WP_Widget_eme_list extends WP_Widget {
   ...
   public function form( $instance ) {
      ...
      if (empty( $instance['format_tpl']) && empty( $instance['format'] )) {
         $format = eme_sanitize_html(DEFAULT_WIDGET_EVENT_LIST_ITEM_FORMAT);
      } elseif (empty( $instance['format_tpl']) && !empty( $instance['format'] )) {
         $format = eme_sanitize_html($instance['format']);
      } else {
         $format = "";
      }
...
?>
  ...
  <p>
    <label for="<?php echo $this->get_field_id('format_tpl'); ?>"><?php _e('List item format','eme'); ?>:</label>
    <?php _e('Either choose from a template: ','eme'); echo eme_ui_select($format_tpl,$this->get_field_name('format_tpl'),$templates_array); ?>
  </p> 
  <p>
    <label for="<?php echo $this->get_field_id('format'); ?>"><?php _e('Or enter your own (if anything is entered here, it takes precedence over the selected template): ','eme'); ?>:</label>
