...
<h1><?php echo $scope_names[$scope]; ?></h1>
   <?php
      admin_show_warnings();
   ?>
...
<script type="text/javascript">
   ...
   <?php if ($pref == "recurrence" && $event['recurrence_freq'] == 'specific') { ?>
      ...
      <?php foreach (explode(',',$event['recurrence_specific_days']) as $specific_day) { ?>
	      mydates.push(jQuery.datepick.newDate(<?php echo $eme_date_obj->setTimestampFromString($specific_day)->format('Y,n,j'); ?>));
      <?php } ?>
...
function eme_admin_event_boxes() {
   ...
   foreach ($screens as $screen) {
        if (preg_match("/$plugin_page/",$screen)) {
           ...
           add_meta_box("div_event_registration_cancelled_email_body", __('Registration Cancelled Email Format', 'eme'), "eme_meta_box_div_event_registration_cancelled_email_body",$screen,"post");
           add_meta_box("div_event_registration_denied_email_body", __('Registration Denied Email Format', 'eme'), "eme_meta_box_div_event_registration_denied_email_body",$screen,"post");
...
?>
   <p><?php _e ( 'The format of the email which will be sent to the respondent if the registration is cancelled.','eme');?>
   <br />
   <?php _e ('Only fill this in if you want to override the default settings.', 'eme' );?>
   </p>
   <?php _e('Either choose from a template: ','eme'); echo eme_ui_select($event['event_properties']['event_registration_cancelled_email_body_tpl'],'eme_prop_event_registration_cancelled_email_body_tpl',$event['templates_array']); ?><br />
   <?php _e('Or enter your own (if anything is entered here, it takes precedence over the selected template): ','eme');?><br />
...
?>
   <p><?php _e ( 'The format of the email which will be sent to the respondent if the registration is denied.','eme');?>
   <br />
   <?php _e ('Only fill this in if you want to override the default settings.', 'eme' );?>
   </p>
   <?php _e('Either choose from a template: ','eme'); echo eme_ui_select($event['event_properties']['event_registration_denied_email_body_tpl'],'eme_prop_event_registration_denied_email_body_tpl',$event['templates_array']); ?><br />
   <?php _e('Or enter your own (if anything is entered here, it takes precedence over the selected template): ','eme');?><br />
...
function eme_admin_enqueue_js(){
   ...
   if ( in_array( $plugin_page, array('eme-registration-approval','eme-registration-seats','events-manager','eme-people') ) ) {
      ...
      wp_enqueue_script('eme-datatables-buttons');
      wp_enqueue_script('eme-datatables-buttons-print');
      wp_enqueue_script('eme-datatables-buttons-html5');
      wp_enqueue_script('eme-datatables-buttons-colvis');
      wp_enqueue_style('eme-jquery-datatables-css',EME_PLUGIN_URL.'js/jquery-datatables/css/jquery.dataTables.css');
      wp_enqueue_style('eme-datatables-colreorder-css',EME_PLUGIN_URL.'js/jquery-datatables/extensions/ColReorder/css/colReorder.dataTables.css');
      wp_enqueue_style('eme-datatables-buttons-css',EME_PLUGIN_URL.'js/jquery-datatables/extensions/Buttons/css/buttons.dataTables.css');
   }

   // add our own css last, so we can override the others if wanted
